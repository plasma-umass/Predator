...
//  fprintf(stderr, "doit function in child thread %d\n", t->tid);
...
//    handle = dlopen("./memt.so", RTLD_NOW);
  ...
  fprintf(stderr, "main line %d, junk1 %p\n", __LINE__, junk1);
    for (i = 0; i < MEMSIZE; i++)
        junk1[i] = i;
  fprintf(stderr, "main line %d\n", __LINE__);
  ...
  fprintf(stderr, "ti malloc addr %p. size CMPTHREAD %ld total %ld\n", ti, sizeof( struct CMPTHREAD ), nthreads * sizeof( struct CMPTHREAD ));
  ...
  fprintf(stderr, "mem malloc addr %p\n", mem);
...
fprintf(stderr, "pdv malloc addr %p\n", pdv);

        errflag = (int *)my_malloc(sizeof(int));
        *errflag = 0;

//      exit(0);
      fprintf(stderr, "%d: before initialization on each thread\n", getpid());
