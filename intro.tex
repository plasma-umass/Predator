...
(accidentally) eliminates false sharing in the Phoenix
linear\_regression benchmark at certain optimization levels, while
LLVM does not at any optimization level.  A slightly different memory
...
false sharing problems that could appear in a slightly different
environment.
...
not manifest in an execution but may appear and greatly degrade the performance of programs 
in a slightly different environment. 
Predictive false sharing detection overcomes a key limitation of previous detection tools, which can miss problems that can easily occur due to minor changes in the execution environment.
