...
#define MAX_USER_SPACE    (0x240000000)
...
void handleAccessBefore(unsigned long addr, int isWrite, int typesize);

//__attribute__ ((always_inline)) 
inline void handleAccessBefore(unsigned long addr, int isWrite, int typesize) {
  ...
  //if(isWrite == 1)
  //printf("handleAccessBefore: addr %lx isWRite %d typeSize %d\n", addr, isWrite, typesize);
  ...
  if(totalWrites < THRESHOLD_TRACK_DETAILS) {
    ...
    if(isWrite == 1) {
      totalWrites = 1;
      asm volatile("lock; xadd %0, %1"
          : "+r" (totalWrites), "+m" (*status)
          : : "memory");
     // totalWrites = atomic_increment_and_return(status);
      if(totalWrites == INVOKE_DETAILED_TRACKING) {
//        fprintf(stderr, "totalWrites %ld at addr %lx allocCacheTrack. index %ld\n", totalWrites, addr, cindex);
    ...
    if(tracking != 0) {
      ...
      *access += 1;
      //fprintf(stderr, "Address %lx Access address %p\n", addr, access);
      if(*access%SAMPLE_INTERVAL <= SAMPLE_ACCESSES_EACH_INTERVAL) {
        handleAccess(addr, typesize, isWrite);
      }
    }
  }
