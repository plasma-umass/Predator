...
benchmark suites, normalized runtime for \defaults{} (with prediction) is around $6.7\times$.
Also, there is no much performance difference for \defaults{} with and without prediction. 
This also means that prediction part of \defaults{} only adds very minimum performance overhead. 
...
For \texttt{bodytrack} and \texttt{ferret}, \defaults{} detected a large amount of cache lines with 
...
Currently, we have no idea about why \texttt{kmeans} runs slow in \defaults{}. 
...
Since \defaults{} do not instrument stack variables, 
